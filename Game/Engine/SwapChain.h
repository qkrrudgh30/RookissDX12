...
// 교환 사슬
// [외주 과정]
// - 현재 게임 세상에 있는 상황을 묘사 해줘야함.
// - 정확히 어떤 공식으로 무엇을 어떻게 계산할지 던져줌
// - GPU가 열심히 계산 (외주)
// - 결과물 받아서 화면에 그려준다
...
public:
    ...
    ComPtr<ID3D12Resource> GetCurrentBackBufferResource() { return _renderTargets[_backBufferIndex]; } // 즉, 현재 백버퍼인 특수 종이를 반환하는 함수.

private:
    ComPtr<IDXGISwapChain>  _swapChain;
    ComPtr<ID3D12Resource>  _renderTargets[SWAP_CHAIN_BUFFER_COUNT];    // SWAP_CHAIN_BUFFER_COUNT: 버퍼 갯수.
    uint32                  _backBufferIndex = 0;                       // 현재 백버퍼(GPU 작업중인 버퍼)는 누구인가를 가르킴. 이 코드에서는 0과 1을 오감.
};